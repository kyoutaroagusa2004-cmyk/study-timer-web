...
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            return data
    return {"total_beans": 0, "logs": [], "unlocked_sounds": ["デフォルトベル"], "current_sound": "デフォルトベル", "unlocked_items": ["丸太のテーブル", "白壁"], "current_items": {"テーブル": "丸太のテーブル", "壁": "白壁", "看板": "なし"}}
...
with tab2:
    ...
    if st.session_state.get("sw_running", False):
        st.info(f"💡 {random.choice(STUDY_TRIVIA)}")
...
# --- 💤 スリープ ---
with tab3:
    st.subheader("💤 スリープタイマー")
    s_min = st.number_input("アラーム（分）", 1, 120, 60)
    s_note = st.text_input("アラームメッセージ", "起きて！")
    if st.button("スリープ開始"):
        ph = st.empty()
...
with tab4:
    ...
    with col_l:
        ...
        for log in reversed(st.session_state.user_data["logs"]):
            st.caption(f"{log['date']} | {log['min']}分")
    with col_r:
        st.subheader("🛒 ショップ")
